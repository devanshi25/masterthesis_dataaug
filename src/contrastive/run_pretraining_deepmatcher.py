...
class DataTrainingArguments:
    ...
    domain_aug: Optional[bool] = field(
        default=False, metadata={"help": "Whether to use the domain data augmentation to use or not."}
    )
    d_aug: Optional[str] = field(
        default=None, metadata={"help": "The domain data augmentation to use."}
    )
    d_aug_prob: Optional[float] = field(
        default=None, metadata={"help": "The domain data augmentation probability to use."}
    )
...
def main():
    ...
    if training_args.do_train:
        ...
        if training_args.load_best_model_at_end:
            ...
            test_dataset = ContrastivePretrainDatasetDeepmatcher(test_dataset, tokenizer=model_args.tokenizer, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, aug_prob=data_args.augment_prob, combo=data_args.combo, domain_aug=data_args.domain_aug, d_aug=data_args.d_aug, d_aug_prob=data_args.d_aug_prob)

    elif training_args.do_eval:
        if "validation" not in raw_datasets:
            raise ValueError("--do_eval requires a validation dataset")
        validation_dataset = raw_datasets["validation"]
        validation_dataset = ContrastivePretrainDatasetDeepmatcher(validation_dataset, tokenizer=model_args.tokenizer, clean=data_args.clean, dataset=data_args.dataset_name, deduction_set=data_args.id_deduction_set, aug=data_args.augment, aug_prob=data_args.augment_prob, combo=data_args.combo, domain_aug=data_args.domain_aug, d_aug=data_args.d_aug, d_aug_prob=data_args.d_aug_prob)
